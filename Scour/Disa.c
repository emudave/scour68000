...
u16 (CPU_CALL *DisaWord)(u32 a)=NULL;

static u32 DisaLong(u32 a)
{
  u32 d=0;
...
int DisaGetEa(char *t,int ea,int size)
{
  ...
  if (ea==0x3a)
  {
    ...
    sprintf(t,"($%x,pc)",ext);
    sprintf(Comment,"; =%x",DisaPc+(s16)ext); // Comment where pc+ext is
    DisaPc+=2;
    return 0;
  }
  ...
  if (ea==0x3b)
  {
    ...
    sprintf(Comment,"; =%x",DisaPc+(s8)off); // Comment where pc+ext is
    DisaPc+=2;
    return 0;
  }
...
{
  int off=(s16)DisaWord(DisaPc); DisaPc+=2;
...
static int DisaDbra(int op)
{
  ...
  // Get offset
  pc=DisaPc;
  Offset=(s16)DisaWord(DisaPc); DisaPc+=2;
...
static int DisaBranch(int op)
{
  ...
  pc=DisaPc;
  Offset=(s8)(op&0xff);
       if (Offset== 0) { Offset=(s16)DisaWord(DisaPc); DisaPc+=2; }
